(function() {
    ...
    Vue.component('position', {
        props: ["name", "key_press", "position", "orientation", "current_midi_note"],
        data: function(){
            return {
                newKey: "",
                newName: "",
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <strong>Key: </strong>{{key_press}} <button class="float-right btn-sm btn-primary mt-1 mr-1" v-if="!editingKey" v-on:click="editKey()">Edit Key</button>
                        ...
                        <div v-if="editingKey">
                            <input id="new-key" type="text" class="form-control" v-model="current_midi_note">
                            <button class="btn-sm btn-primary mt-1 mr-1" v-on:click="updateKey(current_midi_note)">Update Key</button>
                        </div>
                        <div>
                        <strong>Position: </strong>
                        <br>
    ...
    Vue.component('midi', {
        props: ["midi_device", "midi_id", "midi_channel", "midi_devices", "midi_channels"],
        data: function(){
            return {
                showDevices: false,
                showChannels: false
        ...
        template: `
            <div class="card">
                ...
                <div class="card-body"> 
                    <div>
                        <strong>Current Midi Device:</strong> {{midi_device}}
                    </div>
                    <div>
                        <strong>Current Midi Channel:</strong> {{midi_channel}}
                    </div>
                    <div class="dropdown">
                        <input type="button" id="selectedType" v-on:click="toggleDevices()" class="gray mt-3" value="Midi Inputs">
                        <ul class="dropdown-type">
                            <div id="typeDropdown" class="dropdown-items" :class="{ show: showDevices }">
                                <li v-for="midiDevice in midi_devices" v-on:click="sendDevice(midiDevice)">{{ midiDevice }}</li>
                            </div>
                        </ul>
                    </div>
                    <div class="dropdown">
                        <input type="button" id="selectedType" v-on:click="toggleChannels()" class="gray mt-3" value="Midi Channels">
                        <ul class="dropdown-type">
                            <div id="typeDropdown" class="dropdown-items" :class="{ show: showChannels }">
                                <li v-for="midiChannel in midi_channels" v-on:click="sendChannel(midiChannel)">{{ midiChannel }}</li>
                            </div>
                        </ul>
                    </div>
                </div>
