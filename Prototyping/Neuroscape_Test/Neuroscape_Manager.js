...
var DEBUG = false,
    ...
    msInPI = null,
    self,
    url = "https://script.google.com/macros/s/AKfycbw3eTFCHogekvkQsMIJj4-NmbfDsglb_Md81AQaH8iNP0qghzdR/exec";
...
function calculateLatency(collisionTime) {
    ...
    // log(LOG_ARCHIVE, "currentLatency", currentLatency);
    // log(LOG_ARCHIVE, "prevLatency", prevLatency);
    // log(LOG_ARCHIVE, "finalLatency", finalLatency);
...
function makeCallPost(paramObject, timeOutTries){
    var request = new XMLHttpRequest();
    var SUCCESS = "Success";
    ...
    request.ontimeout = function () {
        ...
        if (timeOutTries >= 0) {
            log(LOG_ENTER, "Retrying make call post after timeout");

            makeCallPost(paramObject, timeOutTries - 1);
        }
    };
    request.onreadystatechange = function () {
        ...
        if (request.readyState === 4) {
            log(LOG_VALUE, "request.response", request.response);
            if (request.response !== SUCCESS && timeOutTries >= 0) {
                log(LOG_ENTER, "RETRYING CALL POST");
                makeCallPost(paramObject, timeOutTries - 1);
            } 
        }
...
function prepNextLevel() {
    ...
    // log(LOG_VALUE, "currentMSSpeed: before", currentMSSpeed);
    currentMSSpeed = nextLevel.speed;
    // log(LOG_VALUE, "currentMSSpeed: after", currentMSSpeed);

    // log(LOG_VALUE, "currentGameType: before", currentGameType);
    currentGameType = nextLevel.gameType;
    // log(LOG_VALUE, "currentGameType: after", currentGameType);

    updateGameTypeText(currentGameType);
    // log(LOG_VALUE, "PReP LEVEL BEFOrE CurrenTAV", currentAV);
    currentAV = nextLevel.av;
    // log(LOG_VALUE, "PReP LEVEL AFTER CurrenTAV", currentAV);
...
function startLevel() {
    ...
    currentLevelStartTimeEpoch = Date.now();
...
function stopGame() {
    ...
    var paramObject = {
        ...
        stop: gameData.stop,
        date: gameData.date,
        levels: JSON.stringify(allLevelsData)
    };

    // makeCall(paramObject, 3);
    // console.log("\n\nCalling makeCallPost");
    // makeCallPost(paramObject, 3);
...
function stopLevel() {
    ...
    var totalTime =  (Date.now() - currentLevelStartTimeEpoch) - (COUNT_IN * currentMSSpeed);
    var totalBeats = totalTime / currentMSSpeed;
    var beatsCaptured = collisionCollection.map(function(beat){
        return beat.duringBeat;
    })
    var beatArray = [];
    for (var i = 1; i < totalBeats; i++) {
        beatArray.push(i);
    }
    console.log("beatArray : " + JSON.stringify(beatArray));
    console.log("collisionCollection : " + JSON.stringify(collisionCollection));
    var finalCollisionArray = beatArray.reduce(function(accumulator, current){
        console.log("current : " + JSON.stringify(current));
        console.log(typeof current);
        
        if (collisionCollection.length > 0) {
            console.log("collisionCollection[0]:) " + JSON.stringify(collisionCollection[0].collisionRecord.duringBeat));
            console.log(typeof collisionCollection[0].collisionRecord.duringBeat);
        }

        while (collisionCollection.length > 0 && collisionCollection[0].collisionRecord.duringBeat === 0 ){
            if (collisionCollection[0].collisionRecord.duringBeat === 0) {
                collisionCollection.shift();
            }
        }

        if (collisionCollection.length > 0 && current !== collisionCollection[0].collisionRecord.duringBeat) {
            var record = {
                id: "none",
                collisionRecord: {
                    duringBeat: current,
                    collisionTime: (current + COUNT_IN) * currentMSSpeed
                },
                latency: -1
            };
        ...
        while (collisionCollection.length > 0 && (current === collisionCollection[0].collisionRecord.duringBeat) ){
            accumulator.push(collisionCollection.shift());
        }
    ...
    levelData = {
        ...
        gameType: currentGameType || tempGameType,
        av: currentAV || tempAV,
        startTime: currentLevelStartTime,
        stopTime: new Date(),
        collisionData: finalCollisionArray,
    ...
    if (!nextLevel) {
        ...
        var sendMessage = "Average Latency for the round " + (averageLatency || 0) + "\n";
...
function handleCollision(collisionObject, orbPosition) {
    // log(LOG_VALUE, "HANDLE COLLISION ");
    ...
    if (currentBeat > 0) {
        ...
        if (currentGameType === ON || currentGameType === OFF) {
            ...
            if (isListenMode) {
                ...
                // This is set to null so that nothing plays during replay mode
                currentAV = null;
...
function recordCollision(collisionObject) {
    // log(LOG_VALUE, "recordCollision");
    
    var collisionRecord = new CollisionRecord(
        self.calculateLatency(collisionObject.time),
        collisionObject.time
    );
    
    // var tempLastBeatCounted = lastBeatCounted;
    // console.log("tempLastBeatCounted : " + JSON.stringify(tempLastBeatCounted));
    // console.log("currentBeat : " + JSON.stringify(currentBeat));
    // console.log("lastBeatCounted++ : " + JSON.stringify(lastBeatCounted));
    // if (currentBeat !== lastBeatCounted++) {
    //     var counter = 0;
    //     while (tempLastBeatCounted !== currentBeat) {
    //         console.log("lastBeatCounted++ : " + JSON.stringify(lastBeatCounted));
    //         console.log("currentBeat : " + JSON.stringify(currentBeat));
    ...
    //         var updatedBeat = lastBeatCounted + i;
    //         collisionCollection.push({
    //             id: "none",
    //             collisionRecord: {
    //                 duringBeat: updatedBeat,
    //                 collisionTime: (updatedBeat + COUNT_IN) * currentMSSpeed
    //             },
    //             latency: -1,
    //             time: Date.now()
    //         });
    ...
    // lastBeatCounted = currentBeat;

    if (collisionObject.id === MOUSE_PRESS) {
        // log(LOG_ARCHIVE, "IN RECORD COLLISION about to record mouse press collision ");
